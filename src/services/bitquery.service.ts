...
} from "../models/bitquery.response";

export async function isOwnerRenounced(
  contractAddress: string
): Promise<boolean> {
  const contractOwner = await getOwnerAddress(contractAddress);
  return !!burnAddressesList.find(
    (burnAddress) => burnAddress == contractOwner
  );
}
...
export async function getOwnerAddress(
  ...
  if (
    ...
    smartContractData?.length > 0 &&
    smartContractData[0] != null
  ) {
    return smartContractData[0].smartContract?.attributes?.find(
      (attribute: SmartContractAttributes) => attribute?.name == "owner"
    )?.address?.address;
  } else return undefined;
}

export async function getSmartContractAttributes(
  contractAddress: string
): Promise<BitqueryResponse | null> {
  const query = `
    query {
