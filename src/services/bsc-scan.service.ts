...
export async function getBurnedTokenAmount(
  contractAddress: string
): Promise<string> {
  const accountBalance = async (burnAddress: string) =>
    (await getAccountBalanceOfToken(contractAddress, burnAddress))?.result;
...
export async function getContractTransactions(
  ...
  const url =
    ...
    "&page=1" +
    "&offset=1";
  "&sort=asc" + `&apikey=${process.env.BSC_API_KEY}`;
