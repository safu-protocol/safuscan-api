...
export function checkForExtensions(
  contractSource: string
): ERC20ExtensionsStatus {
  ...
  return new ERC20ExtensionsStatus({
    ...
    pausable:
      extensionsUsed["ERC20Pausable"] || isTokenPausable(contractSource),
...
export const isTokenOwnable = (contractSource: string) =>
  !!contractSource.match("s+is.*Ownable.*{");

export const isTokenMintable = (contractSource: string) =>
  !!contractSource.match("s+_mint(.*)");

export const isTokenPausable = (contractSource: string) =>
  !!contractSource.match("s+is.*Pausable.*{");

export const isTokenProxyable = (contractSource: string) =>
  !!contractSource.match("s+is.*Proxy.*{");
